...
public class HadoopFlightPartitioner extends Partitioner<HadoopFlight, LongWritable> {

	@Override
	public int getPartition(HadoopFlight aHadoopFlight, LongWritable longWritable, int numPartitions) {
		return Math.abs(aHadoopFlight.getUniqueCarrier().hashCode()) % numPartitions;
	}
}